    ...
    return (
      <div className="page">
        <main className="container" style={{ maxWidth: 400, paddingBlock: '4rem' }}>
          ...
          <p className="hint" style={{ marginTop: '1rem' }}>This is a local-only gate for now (not real security).</p>
        </main>
      </div>
    );
  }
    ...
    <div className="page">
      <main className="container" style={{ paddingBlock: '2rem' }}>
        ...
        <div className="admin-actions">
          <button className="btn btn-ghost" onClick={fetchApps} disabled={loading}>{loading ? 'Loading...' : 'Refresh'}</button>
        ...
        {tab === 'applications' && (
          <section>
            ...
            <Link to="/professionals/apply" className="btn" style={{ marginBottom: '1rem', display: 'inline-block' }}>+ New application</Link>
                    ...
                    {apps.map((a) => (
                      <tr key={a.id}>
                        <td>
                          ...
                          <span>{a.phone}{a.email ? ` · ${a.email}` : ''}</span><br />
                          <small>Applied: {new Date(a.created_at).toLocaleString()}</small>
                        </td>
                        <td>{a.role_label ?? a.role}</td>
                        <td>{a.city || '-'}</td>
                        <td>Rs.{a.fee_inr || '-'} · {a.duration_min || '-'}m</td>
                        <td><span className={`status-badge status-${a.status}`}>{a.status}</span></td>
                        <td>
                          <button className="btn btn-sm" onClick={() => setStatus(a.id, 'approved')}>Approve</button>
                          <button className="btn btn-sm btn-danger" onClick={() => setStatus(a.id, 'rejected')}>Reject</button>
                          <button className="btn btn-sm btn-ghost" onClick={() => removeApp(a.id)}>Delete</button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
