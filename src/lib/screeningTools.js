 ...
 * definition below (Pfizer PHQ/GAD, WHO ASRS & AUDIT, WHO-5, Kessler
 * K10, the public-domain PCL-5, and SCOFF). Licensing/permission status
 * per instrument lives in checkEvidence.js — do not claim "validated"
 * in UI copy unless the exact version, scoring, and wording are verified.
 *
 * Tools may carry `status: 'paused'` — paused tools are excluded from all
 * public listings (getAllTools / snapshot / related checks) and their direct
 * route shows an unavailable notice instead of questions or scores.
 *
...
const EXPERIENCE_BY_ID = {
  ...
  bat12: {
    ...
    careGuidance: {
      mild: 'Focus on recovery and workload. Re-check later, or use the mood (PHQ-9) and wellbeing (WHO-5) checks if low mood feels more central.',
...
/** True when a tool must not be publicly offered (listings, snapshot, suggestions). */
...
export function getAllTools() {
  return SCREENING_TOOLS_ENRICHED.filter((t) => !isToolPaused(t));
}

export function toolsForCategory(categoryId) {
  const live = getAllTools();
  if (!categoryId || categoryId === 'all') return live;
  return live.filter((t) => t.category === categoryId);
}
...
export function careTier(tool, result) {
  ...
  if (/severe|higher risk|higher concern|possible dependence|positive screen|consistent with|above a provisional|very low/i.test(label))
    return 'high';
