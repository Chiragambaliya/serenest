...
async function saveApplicationInbox(row, dbError) {
  if (!supabase) return null;
  const payload = {
 ...
 * Used to gate clinician-only professional tools. Returns only a boolean, no profile data.
 */
...
// so unknown blog/tool URLs return a real 404 instead of a soft-200.
...
const GONE_PATTERNS = [
  ...
  /^\/tag(\/|$)/i,
  /^\/wp-/i,
  /^\/academy(\/|$)/i, // retired Serenest Academy catalogue
];
